...
#count the number of sequences with a probability of having an AMP greater than or equal to 0.80
#convert the probability colum to a list 
AMPprobList=fulldataframe['probability_AMP'].tolist()

#TESTING count how many total probabilities there are 
#print(len(AMPprobList))

#create a blank list to add probabilites >= 0.80 to 
LargeAMPprobList=[]

#create a for loop loking for large probabilities, and append them to the previously made list
for value in AMPprobList:
    if value >= 0.80:
        LargeAMPprobList.append(value)

#count the number of values within the list and print
LargeAMPprobCount = len(LargeAMPprobList)
print(LargeAMPprobCount)
